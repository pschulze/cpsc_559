...
public class BidDao implements Dao<Bid, Integer> {
  ...
  public Bid save(Bid bid) {
    ...
    try {
      ...
      PreparedStatement preparedStatement =
          connection.prepareStatement("INSERT INTO bids (auction_id, amount) VALUES (?, ?) RETURNING *");
  ...
  public Bid update(Bid bid) {
    ...
    try {
      Connection connection = Database.getConnection();
      PreparedStatement preparedStatement =
          connection.prepareStatement("UPDATE bids SET auction_id = ?, amount = ? WHERE id = ? RETURNING *");
      preparedStatement.setInt(1, bid.getAuctionId());
      preparedStatement.setDouble(2, bid.getAmount());
  ...
  public void delete(Bid bid) {
    try {
      Connection connection = Database.getConnection();
      PreparedStatement preparedStatement = connection.prepareStatement("DELETE FROM bids WHERE id = ?");
  ...
  public List<Bid> bidsForAuction(Auction auction) {
    ...
    try {
      Connection connection = Database.getConnection();
      PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM bids WHERE auction_id = ?");
  ...
  private Bid bidFromResultSet(ResultSet resultSet) throws SQLException {
    ...
    return new Bid(id, auctionId, amount);
  }
}
